...
void RpcProvider::Run(int nodeIndex, short port) {
    //获取可用ip 定义必要的网络结构体。调用 gethostname 将当前运行机器的主机名存入 hname 数组中。
    char *ipC;
    char hname[128];
    struct hostent *hent;
    gethostname(hname, sizeof(hname));
    // 这一行更进一步。拿到名字（hname）后，它去系统的网络数据库（如 /etc/hosts 或 DNS）中查找这个名字对应的详细信息。
    hent = gethostbyname(hname);
    // h_addr_list：这是一个字符指针数组，里面存储了该主机所有的 IP 地址。
    // 一台电脑可能有多个网卡（比如 WiFi 和有线网），每个网卡都有 IP。
    for (int i = 0; hent->h_addr_list[i]; i++) {
      // inet_ntoa 的含义：全称是 "Internet Network to ASCII"。它的作用是把网络字节序的二进制 IP 转换为字符串（ASCII 码）。
    ...
    /*
      bind的作用：
      如果不使用std::bind将回调函数和TcpConnection对象绑定起来，那么在回调函数中就无法直接访问和修改TcpConnection对象的状态。
...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp) {
    ...
    /*
      ArrayInputStream 是一个适配器。
      它接收数据的 首地址（data()）和 长度（size()）。
      它的作用是告诉后续的工具：“数据就在这段内存里，你去这里读。”
      这是一个 零拷贝（Zero-copy）操作，它不会复制数据，只是“指向”了那块内存
    */
    google::protobuf::io::ArrayInputStream array_input(recv_buf.data(), recv_buf.size());
    /*
      问题：虽然有了数据流，但我们需要读懂里面的 二进制格式（比如 Varint 编码、Tag-Value 结构）。普通的 read() 函数只能读原始字节，读不懂这些高级格式
      解决：CodedInputStream 是 Protobuf 提供的 高级阅读器。
      它包裹了 array_input。
      它拥有特殊技能，比如 ReadVarint32（读变长整数）、ReadString（读字符串）、ReadTag（读字段标签）。
    */
